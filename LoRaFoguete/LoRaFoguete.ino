...
long packetnum = 0;  // contador do numero de pacotes enviados
...
void setup() 
{
  ...
  packetnum = EEPROMReadlong(EEPROMADDRESS);
...
void EEPROMWritelong(int address, long value)
      ...
      EEPROM.write(address, value & 0xFF);//four
      EEPROM.write(address + 1, (value >> 8) & 0xFF); //three
      EEPROM.write(address + 2, (value >> 16) & 0xFF); //two
      EEPROM.write(address + 3, (value >> 24) & 0xFF); //one
      }

long EEPROMReadlong(long address)
      {
      //Le o endereco + os 3 consecutivos
      return ((EEPROM.read(address) << 0) & 0xFF) + (EEPROM.read(address + 1 << 8) & 0xFFFF) + ((long(EEPROM.read(address + 2)) << 16) & 0xFFFFFF) + ((long(EEPROM.read(address + 3)) << 24) & 0xFFFFFFFF);
      }
